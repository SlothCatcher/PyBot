...
def main():
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        from agents.features import MOVE_EFFECT_BLOCK_SIZE, TYPE_MATCHUP_BLOCK_SIZE
        new_cols = int(m.get("arch/obs_dim", 0)) - 715
        check("метрика: новые колонки == урон + типы соперника + хвост оценки приёмов",
              new_cols == DAMAGE_BLOCK_SIZE + TYPE_MATCHUP_BLOCK_SIZE + MOVE_EFFECT_BLOCK_SIZE,
              f"{new_cols} vs {DAMAGE_BLOCK_SIZE} + {TYPE_MATCHUP_BLOCK_SIZE} + {MOVE_EFFECT_BLOCK_SIZE}")
