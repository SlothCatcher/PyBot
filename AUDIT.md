...
## 30. Обрыв BC без traceback на большом датасете: три аллокации на весь датасет

**Симптом.** Прогон шага 1 (`--dataset-path models/mixed.npz`, 3 271 527 x 991, Windows)
обрывался сразу после `BC: оптимизатор создан заново` — без traceback, без исключения, просто
конец вывода. Так убивает процесс Windows, когда пик памяти упирается в коммит-лимит: сообщения
печатались до аллокации, которая не влезла. Сам BC-код при этом корректен — воспроизведено в
Linux: `in_memory` на датасете 250k x 991 даёт OOM-kill (exit 137), то есть исключения и не
должно быть.

**Причина — три независимые аллокации размера датасета** (на 3.27M x 991 obs в float32 = 13 ГБ):

1. `warm_up_vec_normalize` отдавал массив в SB3-шный `RunningMeanStd.update(arr)`, а тот считает
   `np.mean(arr, axis=0)` и `np.var(arr, axis=0)` по всему массиву: `np.var` поднимает временный
   массив того же размера (~13 ГБ) — этого и не было в старом аудите «61GB swap»;
2. нормализация делала `np.empty(obs_arr.shape)` на весь датасет (+13 ГБ);
3. `load_dataset` (его зовёт `run()` на `--dataset-path`) проверял `data["obs"].shape[0]` — а numpy
   для `.npz` читает член ЦЕЛИКОМ: `NpzFile.__getitem__` не принимает `mmap_mode` и не
   транслирует его в `format.read_array` (в numpy 2.4 в `mmap_mode` внутри zip — no-op). То есть
   ещё +13 ГБ ДО того, как что-либо из BC начиналось.

**Что сделано** (`agents/training.py`):

* `warm_up_vec_normalize` считает статистику чанками (`chunk_rows=20 000`): `RunningMeanStd.update`
  вызывается на куске, моменты объединяются взвешенно и совпадают с «одним куском»
  (mean Δ 2.7e-8, var Δ 5.3e-6);
* режим нормализации `auto`: `in_memory` (как раньше) если obs <= `BC_MATERIALIZE_LIMIT_BYTES`
  (0.5 ГБ), иначе `on_the_fly` — нормализация батча через `bc_obs_normalizer` (формула 1:1 с
  SB3 `normalize_obs`, max|Δ| = 0), без копии датасета. Значения `mean/var` warm-up остаются теми
  же, поэтому чекпоинты и инференс не разъезжаются;
* `bc_obs_array()` — единая точка получения obs: `.npy` читается как memmap, большой `.npz` один
  раз выгружается ПОТОКОМ (чанки 64 МБ) в `<датасет>_bc_obs.npy` рядом с датасетом (метаданные в
  `.meta.json`: размер/время/форма/dtype источника → пересоздания при смене источника), дальше
  obs читаются как memmap. Если места на диске не хватает — явное сообщение с ценой в RAM и
  старый путь (obs в RAM), а не молчаливый OOM;
* `load_dataset` читает только заголовок zip (`npz_member_info`) и отдаёт ленивый `_DatasetView`
  (члены читаются лишь при обращении), для `> VIEW_MIN_ROWS` (200 000) — вместе с путём к файлу;
  `pretrain_policy_bc` по view с путём идёт memory-safe маршрутом, warm-up получает уже memmap;
* прогресс BC с ETA, чекпоинт `<save_as>_bc_latest.zip` после каждой эпохи (продолжить:
  `--resume <файл> --bc-keep-optimizer`), `MemoryError`/`KeyboardInterrupt` → сообщение и
  применение лучших весов вместо потери прогона; `--bc-normalize-mode`, `--bc-obs-cache`,
  `--bc-max-examples`, `--bc-progress-every`, `--no-bc-checkpoint`.
  Пороги настраиваются через `PYBOT_BC_MATERIALIZE_LIMIT_GB` и `PYBOT_BC_NPZ_RAM_LIMIT_GB`.

**Замеры (RssAnon-сэмплер, иначе VmHWM путает вытесняемый page cache).** Печатаются в тесте:

| датасет | путь | PEAK_ANON |
|---|---|---|
| 60k x 991 (238 МБ) | `on_the_fly` | 907 МБ |
| 250k x 991 (991 МБ) | `in_memory` (старое поведение) | **OOM, exit 137** |
| 250k x 991 | `on_the_fly` без кэша | 2449 МБ (obs в RAM) |
| 250k x 991 | `on_the_fly` + memmap-кэш | **1506 МБ** |
| 250k x 991 | полный CLI-маршрут `--dataset-path` | **723 МБ** |
| выгрузка в кэш (238 МБ и 991 МБ) | потоком, чанки 64 МБ | 424 МБ (не зависит от датасета) |

То есть пик памяти теперь не масштабируется с датасетом: на 13 ГБ obs он такой же, как на 1 ГБ.
Плата — одноразовый несжатый кэш `models/mixed_bc_obs.npy` (~13 ГБ) рядом с датасетом: он в
`.gitignore`, удаляется в любой момент (пересоздастся) и отключается `--bc-obs-cache off`.

**Проверки** — `test_bc_large_dataset.py` (62 проверки, без сервера):
A) статистика чанками (в `update` никогда не больше `chunk_rows` строк, результат как «один кусок»);
B) `on_the_fly` не материализует датасет (запрет аллокаций > 100 МБ) и даёт тот же батч, что
`in_memory`; C) `auto` выбирает режим по порогу; D) `--bc-max-examples` урезает батчи; E) прогресс
с flush и ETA; F) чекпоинт по эпохам создаётся и грузится; G) `MemoryError`/`KeyboardInterrupt` →
лучшие веса, `interrupted`; H) CLI-флаги доезжают до `run()`; I) `.npz` не грузится в RAM:
заголовок, потоковый кэш без аллокаций > 100 МБ, побайтовое совпадение, переиспользование кэша,
одинаковый loss по обоим путям; J) CLI-маршрут `load_dataset -> _DatasetView -> pretrain`: view
ленивый (0 МБ на 0.12 ГБ obs), BC по view идёт через кэш, `warm_up` по пути не читает obs в RAM,
`--bc-obs-cache off` остаётся старым путём.

---

## 31. Признаки приёмов стояли не в тех слотах, которым соответствуют действия (действия 6..9 / 22..25)

**Симптом (нашёл пользователь на ревью).** Признаки приёмов строились циклом
`for i, move in enumerate(battle.available_moves)`, а poke-env `SinglesEnv` нумерует действия 6..9
по ДРУГОМУ списку:

```python
# poke_env/environment/singles_env.py (action_to_order / order_to_action / get_action_mask)
...
`battle.available_moves` — не «те же приёмы, что в мувсете», а ТОЛЬКО разрешённые сейчас, короче
и без «дыр» (`Pokemon.available_moves_from_request` отбрасывает всё с `disabled: true`; так сервер
отдаёт PP=0, Disable, Taunt, Choice-lock, Heal Block, Torment). Пример: ход №2 задизейблен →

```
known_moves     = [A, B(disabled), C, D]     маска: действия 6, 8, 9   (слоты 0, 2, 3)
available_moves = [A, C, D]                  признаки (было): слот 0=A, 1=C, 2=D, 3=дефолт (-1)
```

То есть действие 8 (приём C) читало характеристики D, действие 9 (D) — дефолтные `-1/1`, а
«родные» признаки B не видел никто. Расхождение возникало ровно в те ходы, когда часть мувсета
недоступна, — и сеть получала несогласованную картину: характеристики в слотах не соответствовали
её же легальным вариантам выбора.

**Где было.** Три места (все — на живом poke-env 0.16.1 воспроизведены):

| место | что ломалось |
|---|---|
| `features.embed_battle_with_fusion` (цикл приёмов) | `moves_base_power/dmg_multiplier/wasted/accuracy/pp/flags` — весь блок приёмов |
| `features._damage_block` (`moves = available_moves[:4]`) | урон по слотам `out[3*i]` — не тот приём |
| `diagnose_type_spam._log_decision` (`action < len(available_moves)`) | диагностика приписывала ходу чужой приём (та же конвенция DoublesEnv, что уже чинили в `env.action_to_order`) |

**Фикс.** Одна функция-эталон `features.move_slots_for_action(battle)` (плюс `move_slot_index` и
`available_move_ids`) повторяет логику poke-env 1:1, включая редкий случай «доступен ровно один
приём, которого нет среди известных». На неё переведены: признаки приёмов, блок урона,
`env._moves_for_action` (бухгалтерия награды; теперь это тонкая обёртка — раньше логика была
продублирована), `policy_player_simple.embed_battle`, `diagnose_type_spam`. Признаки приёмов
считаются по слотам `known_moves[:4]`, а для приёма, которого нет в `available_moves`, ставится
`moves_wasted[i] = 1.0` («есть в мувсете, но сейчас недоступен» = семантически бесполезен),
остальные характеристики приёма остаются в своём слоте. Пустые слоты (недораскрытый мувсет
оппонента) сохраняют прежние дефолты (`base_power = -1`, `accuracy/pp = 1`).

**Эффект (реальный пример, heatran Fire/Steel, earthquake disabled):**

```
base_power  было: [1.1, 0.9, 0.0, -1.0]   стало: [1.1, 1.0, 0.9, 0.0]      (слоты = known_moves)
wasted      было: [0,   0,   0,   0  ]    стало: [0,   1,   0,   0]        (выключенный приём)
```
В этом состоянии расходились 24 колонки obs из 991. Изменений размерности нет — `N_FEATURES`
остаётся 991, миграция чекпоинтов не нужна. Хеш кода признаков (`_features_fingerprint`)
меняется автоматически, поэтому кэш датасета и сайдкар статистики нормализации считаются
устаревшими и пересчитываются (это и задумано).

**Проверки** — `test_move_slots.py` (45 проверок) на НАСТОЯЩИХ объектах poke-env (бой собирается
из сообщений и `request`, маска и `action_to_order` — из poke-env):

* A) для каждого легального по маске действия 6..9/22..25 приём из `action_to_order` совпадает
  со слотом `move_slots_for_action` (случаи: все приёмы доступны, выключен 1-й/2-й/3-й/4-й,
  с тера-действиями, со скамейкой); выключенные слоты запрещены маской;
* B) `obs[0:4]` (base_power), `obs[4:8]` (множители типов) и `obs[8:12]` (wasted) описывают
  именно приёмы слотов: при выключенном earthquake слот 1 = earthquake (bp 1.0), слот 2 = icebeam
  (0.9), при этом `wasted[1] = 1.0` — и проверено, что `_move_wasted_flag` для этого приёма сам по
  себе 0.0 (т.е. единица стоит из-за недоступности, а не из-за иммунитета);
* C) когда доступны все приёмы, obs совпадает с прежним поведением (регресс не задет);
* D) `_damage_block` вызывает `estimate_damage` ровно по приёмам слотов;
* E) редкий случай poke-env (единственный доступный приём, которого нет среди известных) — слот 0;
* F) недораскрытый мувсет (2 известных приёма) и отсутствие активного покемона — не падаем,
  хвостовые слоты остаются дефолтными;
* G) `env._moves_for_action` и `features.move_slots_for_action` дают один и тот же список.

Демонстрация именно этого бага на старом коде (одно выключенное действие):

```
НОВЫЙ код  obs[0:4] base_power : [1.1 1.  0.9 0. ]
СТАРЫЙ код obs[0:4] base_power: [ 1.1  0.9  0.  -1. ]
НОВЫЙ код  obs[8:12] wasted   : [0. 1. 0. 0.]
СТАРЫЙ код obs[8:12] wasted   : [0. 0. 0. 0.]
```

**Замечание про уже собранные данные.** Датасеты/heuristic-кэш, собранные ДО этой правки, в
строках, где часть мувсета была недоступна, содержат признаки в старых слотах (для ходов со всем
доступным мувсетом данные идентичны). Сайдкар статистики нормализации после правки не совпадёт по
хешу — статистика будет пересчитана автоматически.

## 32. Второй режим работы: выбор привязан к содержанию кандидата, а не к индексу слота

**Зачем.** В режиме `indices` (историческое поведение poke-env) номер действия означает позицию:
`6..9` — «слот приёма i», `0..5` — «i-й монстр в порядке `battle.team.values()`». Позиция ничего
не говорит о содержании, а порядок команды в random battle генерируется заново каждый бой:
индекс 2 в одном бою — Garchomp, в другом — Ferrothorn. Пункт (cc) выровнял слоты приёмов
(§31), но сама привязка «позиция = смысл» осталась — и для свитчей она выражена сильнее всего.

**Что сделано.** Появился `--action-mode {indices,embed}` (по умолчанию `indices`, старое
поведение не меняется; для тестов/скриптов — env-переменная `PYBOT_ACTION_MODE`).

* `agents/features.py`: раскладка obs формализована — `OBS_BLOCKS` (58 блоков), `obs_layout()`,
  `verify_obs_layout()` (сверяет сборку `embed_battle_with_fusion` по именам и ширинам и падает
  при расхождении), `_index_maps()` + `move_feature_index()` (4×33), `switch_feature_index()`
  (5×52 = 40 bench + 12 угроз), `context_feature_index()` (599), `context_dim()`,
  `move_candidate_matrix()`, `tera_candidate_matrix()`, `switch_candidate_matrix()`,
  `candidate_slices_ok()`. Индексные карты — единственный источник срезов: модель и `obs`
  не могут разъехаться по построению.
* `agents/action_space.py`: нумерация действий для обоих режимов, `set_action_mode/reset/get`,
  `embed_reserves()` (канонический порядок резервов — тот же, что в bench-блоке),
  `get_action_mask()`, `action_to_order()`/`order_to_action()`. В embed свитч-действие `j` —
  это j-й резерв канонического порядка; при `battle._wait` возвращается базисная маска/ордер
  poke-env, при выходе за число резервов — `ValueError` (никаких молчаливых заглушек).
* `agents/policy.py`: `EmbeddedActorCriticPolicy` — `ctx_encoder` (599→256), `move_encoder`
  (34→128: 33 признака приёма + флаг «с тера»), `switch_encoder` (52→128), `score = MLP([ctx; emb])`
  (384→256→1) для каждого кандидата, `value_net_embed` (261→256→1). Легаси-ветки
  (`features_extractor`/`mlp_extractor`/`action_net`) сохраняются для механик SB3
  (state_dict, миграция, `predict`), но в выборе действия не участвуют и строятся крошечными.

**Про переменную длину кандидатов.** Набор кандидатов переменный (4 приёма + 4 тера-варианта +
до 5 свитчей), но **действие остаётся числом 0..25**, а распределение — softmax по 26 логитам с
маской: недоступные кандидаты получают `-inf` (аддитивная маска), недоступные в gen9-фьюжне слоты
(5, 10..21) — пол `-1e9`. Это математически то же самое, что softmax по множеству доступных
кандидатов, но не требует variable-length `Categorical` и паддинга батча — то есть класса тонких
багов, из-за которых importance ratio в PPO мог бы «тихо» стать неверным. Инвариантность к
перестановкам обеспечивается тем, что логит кандидата зависит только от `(контекст, признаки
этого кандидата)`: тест D переставляет признаки двух приёмов и проверяет, что их логиты
переставляются вместе с ними (у `indices`-политики такого свойства нет).

**Режим — свойство модели, а не флаг запуска.**

* `agents/checkpoint_utils.action_mode_from_checkpoint()`: `policy_kwargs['action_mode']` →
  веса (`ctx_encoder`/`switch_encoder`/`action_net_move`) → иначе `indices` (исторические
  чекпоинты), `None` — если файл вообще не прочитался.
* `agents/policy_player.py`: `--action-mode` задаёт режим; при `--resume` режим берётся из
  чекпоинта (и печатается, если CLI просил другое), при этом `resume_migration_target()`
  не тащит embed-чекпоинт в «миграцию старой архитектуры» (см. ниже), а страховка падает с
  понятным `SystemExit`, если режим embed, а голов кандидатов в модели нет.
* `index.py` (инференс): режим применяется из чекпоинта до создания ботов — иначе маска и
  ордера разъехались бы с тем, что видела политика при обучении.
* Датасеты: `save_dataset()` пишет сайдкар `<ds>.meta.json`
  `{obs_dim, mask_dim, action_mode, features_hash, saved_at}`; `validate_bc_dataset()`
  отвергает датасет чужого режима с объяснением (SystemExit), датасет без сайдкара = `indices`.
  Сборщик replay-датасетов (`collect_replay_dataset.py`) явно помечает свои файлы `indices`:
  метки свитчей там считаются по порядку `battle.team`, и в embed такие метки означали бы другое.

**Что нашли и починили по ходу** (те самые «тонкие места»):

* `_index_maps()` собирал колонки приёмов плоским списком и `reshape(4, 33)` перемешивал
  слоты с признаками: `base_power` кандидата 3-го слота оказывался 0.0. Теперь карта
  собирается по слотам (`obs[|MOVE_FEATURE_IDX[i]|]` == признаки i-го приёма); ловит тест C.
* resume: проверка архитектуры выполнялась **до** определения режима. Embed-чекпоинт имеет
  `features_dim=64` (легаси-ветки намеренно крошечные), поэтому уходил в ветку «старая
  архитектура»: миграция, сброс optimizer state, режим не переключался — env играл в `indices`
  против embed-модели. Режим теперь определяется первым, а `resume_migration_target()`
  для embed мигрирует только по несовпадению obs.
* Недоступные слоты получали `-inf` в логитах: печать метрик давала `inf`, а при неудачной
  маске softmax мог получить NaN. Заменено на `-1e9`.
* `--save-as /abs/path` склеивался с `models/` (`models//abs/path.zip`); теперь абсолютный путь
  уважается, а хвостовой `.zip` не удваивается.

**Контрольные замеры и смоук.**

* Живые бои (80 боёв gen9randombattle, 4839 решений агента): часть приёмов недоступна в 14.4%
  решений, «дыра» (разрешённый слот после выключенного — то, что сдвигало признаки до §31) —
  4.6%; причины: 28 решений из-за PP, 667 из-за блокировок (Disable/Taunt/Choice-lock и пр.).
  То есть сдвиг слотов — не редкость (на офлайн-датасете было 2.6%), а блокировки преобладают.
* CLI-смоук надежды embed-режима: сбор 2 боёв → `ds.npz.meta.json` c `action_mode: embed` →
  BC 1 эпоха → чекпоинт; `--action-mode indices` при `--resume` этого чекпоинта напечатал
  «Режим действий взят из чекпоинта: embed», миграции не было, моменты SplitAdam сохранились.
  (Бои — на `gen9randombattle`: серверный мод `gen9fusionmonsrandombattle` в песочнице
  отсутствует, фьюжн-ветки при этом уходят в dex-fallback, как и задумано.)

**Тесты.** `test_slot_free_mode.py` — 39 проверок: A) нумерация свитчей embed и round-trip
`action_to_order`/`order_to_action`; B) инвариантность к порядку команды (в `indices` — нет);
C) срезы кандидатов = признаки конкретных кандидатов, покрытие obs; D) перестановка признаков
переставляет логиты (и сравнение с `indices`); E) PPO `collect_rollouts`+`train` и BC в embed,
плюс softmax/entropy/`log_prob` строго по разрешённым действиям (`evaluate_actions.log_prob`
== `log_prob` действий — важное для importance ratio); F) режим в чекпоинте/сайдкаре/резюме.
Общая батарея — 22/22.

**Ограничения.** Датасеты, собранные до этой правки в режиме `indices`, в embed не годятся
(сайдкар помешает перепутать); replay-датасеты принципиально `indices`; `N_FEATURES` (991) и
раскладка obs не менялись ни в одном из режимов, так что старые чекпоинты грузятся как раньше
(режим у них определяется как `indices`).

---

## 33. Полная оценка приёмов: N_FEATURES 991 -> 1119 (только хвост)

**Симптом.** Модель не различала приёмы, которые не наносят урона по-разному: Swords Dance и
Howl выглядели одинаково (+1 vs +2 atk), Close Combat не показывал, что снижает СВОИ def/spd,
Rest не был виден как «сон на себя», Solar Beam — как зарядка, Hyper Beam — как перезарядка,
Revival Blessing — как возрождение союзника, Stealth Rock / Spikes / Toxic Spikes / Sticky Web —
как разные хазарды, а Bullet Seed / Slash / Ice Punch — как приёмы под Bulletproof / Sharpness /
Iron Fist. Приёмы различались уроном, точностью, PP и флагами, то есть выбирать их осмысленно
модель не могла.

**Что добавлено (128 признаков, все в хвост obs — старые колонки не сдвинуты).**

| Блок | Ширина | Смысл |
|---|---|---|
| `moves_boost_own_stages` | 28 (4x7) | стадии бустов/дебаффов, накладываемых на НАШУ сторону: atk/def/spa/spd/spe + accuracy/evasion; secondary-эффекты взвешены шансом |
| `moves_drop_opp_stages` | 28 (4x7) | стадии изменений статов СОПЕРНИКА (Growl/Memento) |
| `moves_status_self` | 4 | шанс наложить статус НА СЕБЯ (Rest = 1.0) |
| `moves_charge_turns` | 4 | сколько ходов заряжается (Solar Beam/Fly/Dig/Geomancy — 18 приёмов) |
| `moves_recharge` | 4 | тратит следующий ход (Hyper Beam — 10 приёмов) |
| `moves_revive` | 4 | возрождение союзника и с какой долей HP (Revival Blessing = 0.5) |
| `moves_faint_user` | 4 | роняет своего (Explosion/Self-Destruct/Memento/Healing Wish — 7) |
| `moves_drain` | 4 | доля урона, возвращаемая в HP (Giga Drain — 13 приёмов) |
| `moves_hazard_set` | 16 (4x4) | Stealth Rock / Spikes / Toxic Spikes / Sticky Web |
| `moves_ability_flags` | 32 (4x8) | bullet/slicing/bite/punch/pulse/wind/dance/powder (Bulletproof, Sharpness, Strong Jaw, Iron Fist, Mega Launcher, Wind Rider, Dancer, Powder) |

Источник — `Move.entry` из данных Showdown (`boosts`, `self`, `secondary`/`secondaries`, `flags`,
`forceSwitch`, `sideCondition`, `slotCondition`, `selfdestruct`), а не хардкод таблиц. Ранее
существовавшие признаки (heal, status_prob, priority, recoil, phaze, contact, sound, multihit)
не тронуты — это проверяется отдельной частью теста.

**Найденный тестом баг.** Howl нацелен на `allies`, а не на `self`: хелпер считал «нашей»
только цель `self`, поэтому Howl (+1 atk) не давал признака. Теперь наша сторона — это
`self/allies/allySide/allyTeam`.

**Тихий рассинхрон размерности (лечится в этом же коммите).** `N_FEATURES` объявлена в
`config.py`, но цепочка импортов `config -> action_space -> features` циклична, поэтому
`features.py` читал конфиг через `try/except ImportError` с fallback-константой `991`. После
добавления блоков раскладка стала 1119, а «видимое» значение осталось 991 — то есть obs и
проверки разъехались бы молча. Теперь размерность живёт в `agents/dims.py` (модуль без
импортов), `config.N_FEATURES` и `features.N_FEATURES` берут её оттуда, а `obs_layout()`
падает при расхождении с суммой `OBS_BLOCKS`.

**Совместимость.** Старые чекпоинты (991 и меньше) грузятся как раньше: экстрактор строится под
1119, старые веса кладутся в первые колонки, хвост — нули; `_migrate_checkpoint_dim`
(`target_dim=N_FEATURES`) уже это умеет, и метрика `arch/new_cols_rms_ratio == 0` показывает,
что warm-start не «фантазирует» на новых колонках. Датасеты и статистика нормализации
инвалидируются по `features_hash`/размерности автоматически (`stats_verdict`, сайдкары).

**Тесты.** `test_move_effects.py` — 61 проверка: значения по данным Showdown (включая взвешивание
secondary по шансу: Ancient Power = 0.1 по 5 статам), доставка в obs по слотам, пустые слоты
строго нулевые, полный проход по 954 приёмам без исключений, регресс прежних сигналов и
ЗАМОРОЖЕННЫЙ слепок смещений 55 прежних блоков из коммита `fc48248` (доказывает «рост только в
хвост»). Плюс обновлены `test_features_consistency.py` (105), `test_dim_migration.py` (60),
`test_fusion_types.py` (71).

## 34. Винрейт 53% / 1.7%: режим действий не доезжал до env-воркеров

**Симптом.** Модель в режиме `embed`: 53% против случайного бота и 1.7% против эвристики —
уровень случайной игры при внешне здоровом обучении.

**Причина.** В свежей ветке (без `--resume`) `SubprocVecEnv` создавался РАНЬШЕ, чем в главном
процессе выставлялся режим `embed`, а сам режим не передавался в воркеры вообще: `set_action_mode`
менял только состояние родителя. Воркеры стартовали в `indices` и отдавали obs с маской в
нумерации `battle.team`, тогда как политика выбирала действие по каноническому порядку резервов
(`embed_reserves`). Для приёмов (6..9) и теры (22..25) нумерации совпадают, поэтому выбор атаки
выглядел осмысленным, а КАЖДЫЙ свитч вёл на другого покемона: «индекс j» означал то резерв,
то позицию в команде. Обучение и оценка через `SubprocVecEnv` получали шум, объясняющий и
1.7% против эвристики, и «половину» против случайной игры.

**Фикс.** Режим стал свойством окружения-воркера, а не только процесса:

* `action_space.set_action_mode()` экспортирует `PYBOT_ACTION_MODE`, `reset_action_mode()` удаляет —
  режим наследуется потомками (включая `spawn`);
* `ExampleEnv.create_env(opponent_weights, action_mode)` применяет режим ПЕРВОЙ строкой в воркере
  (`_apply_worker_action_mode`), сборка вынесена в `_build_env`;
* `policy_player` выставляет режим из чекпоинта в начале `run()` ДО создания любых окружений, все
  четыре фабрики `SubprocVecEnv` используют `partial(..., action_mode=_mode)`, дубль блока в
  свежей ветке убран;
* при загрузке self-play снапшота режим которого не совпадает с режимом прогона — предупреждение
  (ключ `mode:<файл>`, печатается один раз);
* `play_trained.py` применяет режим из чекпоинта перед созданием игрока (инференс);
* легаси `policy_player_simple.py` на `embed` падает с явной ошибкой вместо молчаливой игры
  позиционной политикой по embed-маске.

**Попутно найдено живьём: forkserver виснет, нужен spawn.** `SubprocVecEnv` без явного
`start_method` берёт `forkserver` (дефолт SB3 на Linux) — и с poke-env-воркерами это мёртвый
висяк: родитель вечно ждёт `get_spaces`, ни ошибки, ни логов, ни таймаута (проверено и на
текущем коде, и на `fc48248` — то есть баг был и раньше и не связан с фиксом режима).
Со `spawn` те же env поднимаются за 1.5 с. Теперь все четыре фабрики в `policy_player`
передают `start_method=_vec_start_method()` → `spawn` (переопределяется
`PYBOT_VEC_START_METHOD`); структурная проверка в тесте следит, чтобы ни одна фабрика не
осталась без него.

**Тесты.** `test_embed_mode_env.py` — 23 проверки: A) экспорт/сброс режима в окружении; B) режим
виден обычному дочернему процессу и `spawn`-процессу (и воркер с `action_mode="embed"` отдаёт
маску в embed-нумерации); C) `create_env(action_mode=...)` включает режим в воркере и не теряет
`opponent_weights`; D) структурная проверка исходника `policy_player` (режим выставляется до
первого `SubprocVecEnv`, каждая фабрика передаёт режим) — ловит возврат бага; E) семантика на
синтетическом бою: `action_to_order(j)` в embed ведёт на j-й резерв канонического порядка и
пишет «switch» в бухгалтерию награды, в `indices` — на team-индекс; F) ЖИВОЙ бой (3 боя против
случайного бота): каждый свитч через embed-нумерацию реально приводит на ожидаемый резерв
(`mismatch == 0`, 159 свитчей).
